...
type MovementPhase = 'arriba' | 'abajo';
type RepResult = 'correcta' | 'muy profunda' | 'muy superficial';
type FeedbackTone = 'neutral' | 'green' | 'amber' | 'red';
...
const DEFAULT_FEEDBACK: TechniqueFeedback = {
  tone: 'neutral',
  message: 'Comienza el movimiento con control',
};

function createTechniqueState(): TechniqueState {
  return {
    phase: 'arriba',
    repCount: 0,
    goodRepCount: 0,
...
function getRepFeedback(result: RepResult): TechniqueFeedback {
  if (result === 'muy profunda') {
    return {
      tone: 'red',
      message: 'Ángulo muy cerrado — cuidado con la articulación',
    };
  }
  if (result === 'muy superficial') {
    return {
      tone: 'amber',
      message: 'No bajaste lo suficiente — ángulo muy abierto',
    };
  }
  return {
    tone: 'green',
    message: 'Ángulo correcto ✓',
  };
}
...
function updateMovementTechnique(state: TechniqueState, nextAngle: number): TechniqueState {
  ...
  if (next.phase === 'abajo') {
    next.currentMin = next.currentMin === null
      ? nextAngle
      : Math.min(next.currentMin, nextAngle);
    next.currentMax = next.currentMax === null
      ? nextAngle
    ...
    if (nextAngle >= MOVEMENT_THRESHOLDS.up && next.currentMin !== null) {
      const result: RepResult = next.currentMin < MOVEMENT_THRESHOLDS.idealMin
        ...
        : next.currentMin > MOVEMENT_THRESHOLDS.idealMax
          ...
          : 'correcta';
      next.repCount += 1;
      if (result === 'correcta') next.goodRepCount += 1;
      next.results = [...next.results, result].slice(-20);
      next.feedback = getRepFeedback(result);
      next.phase = 'arriba';
      next.currentMin = null;
      next.currentMax = null;
    }
  }
...
function calculateHipDeviation(
  ...
  if ([shoulder, hip, ankle].some((point) => (point.score ?? 0) < 0.2)) return null;

  const bodyLength = Math.hypot(ankle.x - shoulder.x, ankle.y - shoulder.y);
  if (bodyLength < 1) return null;
  const horizontalSpan = ankle.x - shoulder.x;
  const progress = Math.abs(horizontalSpan) < 1
    ? 0.5
    : (hip.x - shoulder.x) / horizontalSpan;
  const expectedY = shoulder.y + (ankle.y - shoulder.y) * progress;
  return (hip.y - expectedY) / bodyLength;
}
...
function updatePlankTechnique(state: TechniqueState, hipDeviation: number | null): TechniqueState {
  ...
  if (hipDeviation > 0.08) {
    next.feedback = { tone: 'red', message: 'Sube la cadera' };
  } else if (hipDeviation < -0.08) {
    next.feedback = { tone: 'amber', message: 'Baja la cadera, no la subas tanto' };
  } else {
    next.feedback = { tone: 'green', message: 'Postura correcta ✓' };
  }
  return next;
}
...
function Home() {
  ...
  useEffect(() => {
    if (phase !== 'tracking' || selectedExercise !== 'plancha') return;
  ...
  const processFrame = useCallback(async () => {
    ...
    try {
      ...
      setAngle(nextAngle);
      setTechniqueState(nextTechniqueState);
  ...
  const startCamera = useCallback(async (exerciseId?: ExerciseId) => {
    ...
    setAnglePoints([]);
    setAngleHistory([]);
    const initialTechniqueState = createTechniqueState();
  ...
  const techniqueResultsLabel = techniqueState.results.length
    ? techniqueState.results.map((result, index) => `${index + 1}. ${result}`).join(' · ')
    : '—';
  const hipDeviationLabel = techniqueState.hipDeviation === null
    ? '—'
    : `${techniqueState.hipDeviation > 0 ? '+' : ''}${(techniqueState.hipDeviation * 100).toFixed(1)}%`;
  const goodTechniqueLabel = activeExercise?.id === 'plancha'
    ? techniqueState.feedback.tone === 'green' ? 'Sí' : '—'
    : techniqueState.goodRepCount.toString();
        ...
        <div className="coach-stage">
          ...
          {isActive && (
            <section className="glass-panel active-panel" aria-labelledby="active-title">
              ...
              <div className="session-hud" aria-label="Resumen de sesión">
                <div className="session-hud-card">
                  <span>{activeExercise?.id === 'plancha' ? 'Tiempo' : 'Repeticiones'}</span>
                  <strong>
                    {activeExercise?.id === 'plancha'
                      ? formatDuration(sessionSeconds)
                      : techniqueState.repCount}
                  </strong>
                </div>
                <div className="session-hud-card session-hud-card--good">
                  <span>Buena técnica</span>
                  <strong>{goodTechniqueLabel}</strong>
                </div>
              </div>
              <div className="video-stage" style={{ aspectRatio: videoRatio }}>
                ...
                <div className="angle-hud" aria-live="polite">
                  ...
                  <small>
                    {activeExercise?.id === 'plancha'
                      ? 'Respecto a hombro · tobillo'
                      : activeExercise?.angleLabel ?? 'Esperando puntos'}
                  </small>
                </div>
                <div
                  className={`technique-alert technique-alert--${techniqueState.feedback.tone}`}
                  role="status"
                  aria-live="polite"
                  aria-atomic="true"
                >
                  ...
                  <div className="diagnostic-row">
                   ...
                   <div className="diagnostic-row">
                     ...
                     <dd className={`diagnostic-value ${activeExercise?.id === 'plancha' ? 'diagnostic-value--accent' : ''}`}>
                       {activeExercise?.id === 'plancha' ? 'Isométrica' : techniqueState.phase}
                     </dd>
                   </div>
                   <div className="diagnostic-row">
                     <dt>Mín. rep. actual</dt>
                     <dd className="diagnostic-value">
                       {techniqueState.currentMin === null ? '—' : `${techniqueState.currentMin}°`}
                     </dd>
                   </div>
                   <div className="diagnostic-row">
                     <dt>Máx. rep. actual</dt>
                     <dd className="diagnostic-value">
                       {techniqueState.currentMax === null ? '—' : `${techniqueState.currentMax}°`}
                     </dd>
                   </div>
                   <div className="diagnostic-row">
                     <dt>Repeticiones</dt>
                     <dd className="diagnostic-value diagnostic-value--accent">{techniqueState.repCount}</dd>
                   </div>
                   <div className="diagnostic-row">
                     <dt>Técnica buena</dt>
                     <dd className="diagnostic-value diagnostic-value--success">{goodTechniqueLabel}</dd>
                   </div>
                   <div className="diagnostic-row diagnostic-row--history">
                     <dt>Registro técnica</dt>
                     <dd className="diagnostic-angle-history">{techniqueResultsLabel}</dd>
                   </div>
                   {activeExercise?.id === 'plancha' && (
                     <div className="diagnostic-row">
                       <dt>Desvío cadera</dt>
                       <dd className={`diagnostic-value ${techniqueState.feedback.tone === 'green' ? 'diagnostic-value--success' : 'diagnostic-value--warning'}`}>
                         {hipDeviationLabel}
                       </dd>
                     </div>
                   )}
